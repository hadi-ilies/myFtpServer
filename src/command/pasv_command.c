...
//NOTE inet_ntoa return the ip adress
//TODO why 256 ? --> because you have to print the first and the last bit of tour port
static void	passive_msg(int *new_socket, server_t *server)
{
    ...
    printf("PORT = %d|IP = %s\n", server->port, ip);
    dprintf(*new_socket, "%s Entering Passive Mode (%ld,%ld,%ld,%ld,%d,%d).\r\n", 
...
static int create_passive_socket(client_t *client)
{
	...
	if (getsockname(client->server.sockfd, (struct sockaddr*)&client->server.address, &len) == -1)
